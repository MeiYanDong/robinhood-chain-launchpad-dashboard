...
Robinhood Chain 发射台数据看板。首页先给出成交量龙头、相对平时最活跃的平台和 PAIR 对比
PONS 的结果；经营、估值和代币排行各自进入独立任务页，口径与审计信息按需查看。

仓库代码 `0.18.0` 将发射台前端重构为“今日概览 / 平台经营 / PAIR vs PONS / 平台代币”四个
...
- 平台经营：相对各自正常水平、7 日 / 30 日 / 上线以来交易量，以及成交量、手续费、协议确认
  收入和实际回购；完整应计、实收、回购预算、留存与净利润在审计明细中展开；
- PAIR vs PONS：展示实际价、PONS 规模参考价、偏离、是否可参考、完整公式和逐项输入；
...
偏离，公开的手续费分配比例只进入折叠的研究情景，不出现在主判断区。页面每 60 秒读取一次
