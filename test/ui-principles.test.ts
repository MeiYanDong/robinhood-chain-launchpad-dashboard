...
test("launchpad dashboard is split into four task views and defaults to a concise overview", async () => {
  ...
  assert.match(html, /data-help-title="为什么七日结果放在第一位？"/);
  ...
  assert.match(html, /id="valuation-seven-day-reference"/);
  assert.match(html, /id="valuation-latest-day-reference"/);
  assert.match(html, /按近 7 个完整日交易量估算/);
  assert.match(html, /按最新完整日交易量估算/);
  assert.match(html, /id="valuation-seven-day-formula-definition"/);
  assert.match(html, /id="valuation-latest-day-formula-definition"/);
  assert.match(html, /PONS 价格 ×（PONS 有效供应量 ÷ PAIR 有效供应量）/);
  assert.match(html, /id="valuation-seven-day-equation"/);
  assert.match(html, /id="valuation-latest-day-equation"/);
  ...
  assert.match(html, /id="valuation-input-pons-seven-day-volume"/);
  assert.match(html, /id="valuation-input-pair-seven-day-volume"/);
  assert.match(html, /id="valuation-input-pons-latest-day-volume"/);
  assert.match(html, /id="valuation-input-pair-latest-day-volume"/);
  assert.match(html, /id="valuation-latest-day-volume-heading"/);
  assert.match(html, /id="valuation-policy-seven-day-equation"/);
  assert.match(html, /id="valuation-policy-latest-day-equation"/);
  ...
  assert.match(html, /id="pair-adjusted-seven-day"/);
  assert.match(html, /id="pair-adjusted-latest-day"/);
...
test("economics client keeps unknown, not-applicable, and refresh routes distinct", async () => {
  ...
  assert.match(app, /valuation\?\.sevenDayFormula/);
  assert.match(app, /valuation\?\.latestDayFormula/);
  assert.match(app, /valuation\?\.inputs/);
  assert.match(app, /ponsEffectiveSupply/);
  assert.match(app, /pairEffectiveSupply/);
  assert.match(app, /ponsSevenDayVolumeUsd/);
  assert.match(app, /pairSevenDayVolumeUsd/);
  assert.match(app, /ponsLatestDayVolumeUsd/);
  assert.match(app, /pairLatestDayVolumeUsd/);
  assert.match(app, /latestDayVsSevenDayPercent/);
  assert.match(app, /距最新可统计日期/);
  assert.match(app, /startLabel\.textContent = `\$\{formatUtcDay/);
  assert.match(app, /endLabel\.textContent = `\$\{formatUtcDay/);
  assert.doesNotMatch(app, /startLabel\.textContent = formatDateTime/);
