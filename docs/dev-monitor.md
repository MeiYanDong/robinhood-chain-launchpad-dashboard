...
所有被官方发行事件确认或推断的创建者都会进入候选账本，但飞书采用
`pair_team_wallet_only` 硬白名单：当前只允许已核验的 PAIR 主发行钱包
`0xa15e4ad0dbc8df1715a7b254526252cd93bb1102`。其余 PAIR/Pons/Long 创建者、交易发送者和普通地址，
即使达到 `proven` / `repeat` 或收益表现优秀，也只进入证据层和画像层，不发送飞书。

该钱包身份由 PAIR 官方代币 API 中的 creator 字段和 PAIR 官方发行交易共同约束。官方若更换钱包，
必须先重新核验这两类证据再更新白名单，不能由市场表现自动加入。
...
- 只通知 PAIR 已核验主发行钱包的新发币和真实买入；其它创建者、画像晋级和普通交易地址
  只留在面板，不发飞书；
...
- 每轮发送前重新核验钱包白名单；旧 outbox 中不属于 PAIR 项目方钱包的待发送项会标记为
  `pair_team_only_policy` 并封存，不删除项目或交易事实；
