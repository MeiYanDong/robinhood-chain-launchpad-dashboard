...
所有被官方发行事件确认或推断的创建者都会进入候选账本，但飞书只接受
`canonical_event + high` 的创建者身份：PAIR / pons 必须由官方 Factory / Coordinator 事件中的
indexed creator / deployer 直接给出。Long 当前只能证明“官方发行事件 + 交易发送者”，因此继续进入
证据层和画像层，但不会发送飞书；`proven`、`repeat` 或收益表现都不能替代创建者身份凭证。

通过身份闸门后，只有 `proven` 创建者才可能进入通知层。
...
- 只通知同时通过“高置信官方创建者身份”和反垃圾门槛的 `proven` 创建者新发币，以及其买入
  已核验发射台代币；Long 交易发送者、画像晋级、普通交易地址和无法证明创建关系的地址
  只留在面板，不发飞书；
...
- 每轮发送前重新核验创建者资格；旧 outbox 中不满足新身份规则的待发送项会标记为
  `unverified_project_creator_policy` 并封存，不删除项目或交易事实；
