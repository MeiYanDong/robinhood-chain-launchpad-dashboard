...
60 秒。缓存超过 60 秒但不超过 5 分钟时，会立即返回最后一次成功结果，同时后台重试并以
`x-ledger-query-stale: true` 明确标记；超过 5 分钟后无法更新则返回明确 503。未缓存 GET
最多等待 15 秒。查询并发上限 6，单响应上限 8 MiB，超限明确失败。POST 保留既有刷新能力
与 Nginx 权限，不产生新的交易能力。

预热清单覆盖完整运行时只读合同，以及发射台经营、龙头、PAIR Alpha、PAIR V2、项目方发行
与资金闭环的默认首屏请求。
5 分钟是 collector 短暂忙碌时的硬兜底上限；响应仍保留自身的 `observedAt`、`stale` 和来源
状态，不会把缺失值补成零，也不会把 POST 刷新结果缓存为成功。
